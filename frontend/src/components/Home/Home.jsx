...
const Home = () => {
    ...
    const handleChange = async (event) => {
        ...
        const switchedNotes = await switchStorage(type, notes);
    ...
    const updateNote = async (title, body) => {
        ...
        if (storage === 'local') {
            ...
            const response = await updateNoteInRemoteDB(note);
            if (response.status === 204) {
                setNotes([...notes.slice(0, index), { _id: note.id, ...JSON.parse(note.note) }, ...notes.slice(index + 1)])
            } else if (response.status === 401) {
                navigate('/');
            } else {
                alert("Error updating note");
